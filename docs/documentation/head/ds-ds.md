...
**Table 11.3. `DataSource` Configuration Properties**
  ...
  portNumber     INT       TCP port which the PostgreSQL™ database server
                           is listening on (or 0 to use the default port)
  ...
  sslfactory     STRING    Custom javax.net.ssl.SSLSocketFactory class name (see
                           the section called
...
**Table 11.4. Additional Pooling `DataSource` Configuration Properties**
  ...
  maxConnections       INT      The maximum number of open database connections to
                                allow. When more connections are requested, the
								caller will hang until a connection is returned
								to the pool.
  --------------------------------------------------------------------------------
